    ...
    <div class="h-full bg-secondary rounded-[15px] px-[100px] py-[64px]">
      <div class="flex flex-col gap-[60px] mx-auto">
        <div class="flex gap-[30px]">
          <input v-model="userEquation" @keyup.enter="balanceEquation" class="flex-1 min-w-[225px] rounded-full h-[64px] w-full pl-5 placeholder:text-secondary text-secondary caret-secondary bg-primary" type="text" placeholder="Enter the formula..." />
          <button @click="balanceEquation" class="flex justify-center items-center bg-primary min-w-[64px] w-[64px] h-[64px] rounded-full"><img class="h-[24px] w-auto" src="../assets/search.svg" alt="search"></button>
        </div>
